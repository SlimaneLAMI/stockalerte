...
export default async function OfferDetailPage({ params }) {
  ...
  const [offer, t, tm] = await Promise.all([
    getOffer(id),
  ...
  const TYPE_LABELS = {
    sale:         t('type_sale'),
    promotion:    t('type_promotion'),
    pack:         t('type_pack'),
    'anti-waste': t('type_anti_waste'),
    donation:     t('type_donation'),
  };

  const DELIVERY_MODES = [
    { key: 'pickup',      icon: Store,         label: t('delivery_pickup'),      color: 'text-blue-600 bg-blue-50 dark:bg-blue-900/20' },
    { key: 'reservation', icon: CalendarCheck, label: t('delivery_reservation'), color: 'text-amber-600 bg-amber-50 dark:bg-amber-900/20' },
    { key: 'delivery',    icon: Truck,         label: t('delivery_delivery'),    color: 'text-primary-600 bg-primary-50 dark:bg-primary-900/20' },
  ];
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card p-6 space-y-4">
              <div className="flex items-start gap-3 flex-wrap">
                ...
                {offer.category && (
                  <Badge variant="default">
                    {offer.category.name?.fr || offer.category.slug}
                  </Badge>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 py-4 border-y border-gray-100 dark:border-gray-800">
                <div>
                  <p className="text-xs text-gray-400 mb-1">{t('expires_on')}</p>
                  <p className="text-sm font-medium">{formatDate(offer.expiresAt)}</p>
                </div>
                {offer.quantity !== undefined && (
                  <div>
                    <p className="text-xs text-gray-400 mb-1">{t('quantity_label')}</p>
                    <p className="text-sm font-medium">{offer.quantity} {offer.unit || t('quantity_left')}</p>
                  </div>
                )}
                <div>
                  <p className="text-xs text-gray-400 mb-1">{t('status_label')}</p>
          ...
          <div className="space-y-4">
            ...
            {offer.merchant && (
              <div className="card p-5">
                <h3 className="font-semibold mb-4 text-gray-900 dark:text-white">{tm('about')}</h3>
                  ...
                  <div>
                    ...
                    {offer.merchant.rating > 0 && (
                      <div className="flex items-center gap-1 text-sm text-gray-500">
                        ...
                        {offer.merchant.rating.toFixed(1)} · {offer.merchant.reviewCount} {tm('reviews')}
                      </div>
                    )}
                  </div>
                </Link>
